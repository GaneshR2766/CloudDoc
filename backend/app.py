...
SERVICE_ACCOUNT_FILE = os.path.join(BASE_DIR, "credentials", "calm-aegis-461805-k7-3be5656f0854.json")
BUCKET_NAME = 'bucket-cloud-doc'
...
def verify_google_token(token):
    try:
        idinfo = id_token.verify_oauth2_token(
            token,
            requests.Request(),
            audience="977589759806-m0r2757kf19l372i5p21s1or1emvgbh9.apps.googleusercontent.com"
        )
        return idinfo['email']
    except Exception:
        return None
...
def preview_file(filename):
    # Use 'owner' query param if provided, else default to requesting user
    owner_email = request.args.get('owner') or request.user_email
    user_folder = get_user_folder(owner_email)
    ...
    try:
        ...
        signed_url = blob.generate_signed_url(
            version="v4",
            expiration=timedelta(minutes=15),  # 15 minutes is safe
            method="GET",
